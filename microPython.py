        ...
        if dummy: #テスト用
            ...
            if Logger.logFileName in os.listdir():
                with open(Logger.logFileName, 'r') as fp:
                    ...
                    # wakeupDTとlastUpdateDTの差を見て、復旧するログのインデックスをいい感じに調整する。
                    # めんどくさいので、日を跨いでOFFしていたら全て履歴を捨てて初めから測定する。同じ日のうちの再接続か過去にタイムトリップしたときだけ履歴を読む。
...
'''
Thonny上で日本語のコメントを書くとTonnyがフリーズしてそれまで書いてたコードが消えるので注意！

なんか時間の進み方が遅い。タイマー確認
時計はPartialUpdateする。-> 電圧かけっぱなしにすると電子ペーパーが焼けつくのでだめ。

wifiは、初回はパスワードなしの特定のSSIDで起動してwebサーバでSSIDとパスワードを入力させる画面を起動
-> QRコード(とURL)を表示して、ブラウザでアクセスさせる
-> 入力したら再起動
